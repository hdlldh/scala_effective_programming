  ...
  property("given an empty heap and a list of integers, insert all to heap and repeatly finding and deleting return the intial sequence of intergers") =
    def check(seq: Seq[Int], heap: List[Node]): Boolean =
      if heap.isEmpty && seq.isEmpty then
        ...
        val heapMin = heapInterface.findMin(heap)
        seq.head == heapMin && check(seq.tail, heapInterface.deleteMin(heap))
  ...
  property("finding a minimum of melding of any two heaps should return a minimum of one or the other") =
    forAll(genHeap.suchThat(_.nonEmpty)) { (heap1: List[Node]) =>
      forAll(genHeap.suchThat(_.nonEmpty)) { (heap2: List[Node]) =>
        val firstMin = heapInterface.findMin(heap1)
        val secondMin = heapInterface.findMin(heap2)
        val meldedHeap = heapInterface.meld(heap1, heap2)
