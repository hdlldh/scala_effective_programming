...
final class Wikigraph(client: Wikipedia):
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if (q.isEmpty) WikiResult.successful(None)
        ...
        val ((curDepth, curArticle), remainingQueue) = q.dequeue
        if (curDepth >= maxDepth) WikiResult.successful(None)
        else {
          client.linksFrom(curArticle).flatMap(articleIds => {
            if (articleIds.contains(target)) then WikiResult.successful(Some(curDepth))
            else
              iter(
                visited + curArticle,
                remainingQueue
                  .enqueueAll(
                    articleIds
                      ...
                      .map(articleId => (curDepth + 1, articleId)))
              )
          }).fallbackTo(iter(visited + curArticle, remainingQueue))
        }
      }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    val allArticlePairs = for {
      srcArticle <- titles
      dstArticle <- titles
      if srcArticle != dstArticle
    } yield (srcArticle, dstArticle)

    WikiResult.traverse[(String, String), (String, String, Option[Int])](allArticlePairs) { (srcTitle, dstTitle) =>
      client.searchId(srcTitle).zip(client.searchId(dstTitle)).flatMap { (sourceId, distId) =>
        breadthFirstSearch(sourceId, distId, maxDepth).flatMap { distance =>
          WikiResult.successful(srcTitle, dstTitle, distance)
        }
      }
    }
end Wikigraph
